...
# Define the starting point for cropping
crop_start_x = 988
crop_start_y = 939

# Crop and save all images to the temporary directory
for file_name in os.listdir(source):
    if file_name.endswith((".png", ".jpg", ".jpeg")):
        img_path = os.path.join(source, file_name)
        img = Image.open(img_path)
        width, height = img.size
        # Ensure the cropping points are within the image dimensions
...
# Now that all images are cropped and saved, perform batch detection
results = model.predict(source=temp_dir, save=True, classes=classes, conf=conf_thresh)

# Assuming results is a list of predictions corresponding to each image
for file_name, result in zip(os.listdir(temp_dir), results):
    ...
    print(f'Estimated wait time for image {file_name}: {wait_time} minutes. Number of people: {num_people}')
